...
### 缓存击穿
- 介绍：客户端**多个并发请求服务端的某个数据**，该数据在数据库中，但是在redis中，已经过期了，因此就会直接查询到数据库中。**针对的是一个key**
- 解决方案：
  - 加互斥锁。并发多个请求，只有第一个请求能拿到锁，执行数据库查询，其他请求阻塞，等到第一个请求处理完成之后，直接从缓存中读取。
  - 设置数据不过期。

### 缓存穿透
- 介绍：客户端请求服务端的某个数据，不在redis中，同时也不在数据库中。下次再次获取该数据时，还是没有，每次请求都会走到数据库。
- 解决方法：
  - 接口校验数据的合法性，只有合法的数据，才去缓存或数据库中查询。
  - 缓存空值，在缓存和数据库中都没有查到数据时，在redis中设置该key对应的空值，并设置较短的过期时间，保证同一个key，不会有过多的流量打到数据库。
  - 布隆过滤器。使用布隆过滤器存储所有可能访问的key，不存在的直接过滤。

### 缓存雪崩
- 介绍：基于缓存击穿，即客户端同时访问多个数据，而这些数据在redis中都都过期了，大量的查询落到数据库上。**针对的是多个key**
- 解决方法：
  - 给数据设置不同的过期时间，避免多个数据同时过期。
  - 给热点数据设置永不过期。
  - 互斥锁。同一个key，只有第一个请求能拿到锁，后续请求直接从缓存中读取，同缓存击穿中的解决方法。
...
- 名词解释
  ...
  - veth-pair：虚拟网络设备，两个虚拟网卡组成的一对设备，分别搭载到不同的网络命名空间，用于两个不通的网络命名空间通信。数据总是从一端流入，流向另一端。

- docker网络模式
  - none：给容器创建了独立的网络命名空间，但是没有进行任何网络配置，即没有网卡、ip、路由等。
  - bridge：docker容器启动的默认模式，即给容器创建了独立的网络命名空间，同时会创建veth-pair，配置ip，并将 veth 挂载到 docker0 默认网桥。（docker0 网桥在安装时就会创建）
  - host：不会给容器创建独立的网络命名空间，容器与主机共享网络命名空间。
